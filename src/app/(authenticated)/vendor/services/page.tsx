...
function VendorServices() {
  ...
  const { createService, updateService, deleteService, isSaving } = useManageServices();
  // Null = adding; an id switches the form to editing that row.
    ...
    <div className="grid gap-6 md:grid-cols-2">
      <div>
        ...
        {services?.length ? (
          <div className="space-y-2">
            {services.map((s) => (
              <div key={s.id} className={`rounded-md border p-3 flex items-center gap-3 ${editingId === s.id ? "border-primary bg-primary/5" : "border-border"}`}>
                <div className="flex-1">
                  <p className="font-medium">{s.name}</p>
                  <p className="text-xs text-muted-foreground">{s.duration_minutes} min · {formatNaira(s.price)}</p>
                </div>
                <button onClick={() => { setEditingId(s.id); setForm({ name: s.name, price: String(s.price), duration: String(s.duration_minutes), description: s.description ?? "" }); }} aria-label={`Edit ${s.name}`}>
                  <Pencil className="h-4 w-4 text-muted-foreground hover:text-foreground" />
                </button>
                <button onClick={() => void handleDelete(s.id)} aria-label={`Delete ${s.name}`}><Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive" /></button>
              </div>
            ))}
          </div>
      ...
      <form className="rounded-lg border border-border bg-card p-5 space-y-3 h-fit"
        ...
        <div className="flex items-center justify-between">
          <h2 className="font-display text-xl">{editingId ? "Edit service" : "Add service"}</h2>
          {editingId && (
        ...
        <Button type="submit" disabled={isSaving} className="w-full bg-primary text-primary-foreground hover:bg-primary/90">{isSaving ? "Saving…" : editingId ? "Save changes" : "Add"}</Button>
      </form>
    </div>
  );
}
