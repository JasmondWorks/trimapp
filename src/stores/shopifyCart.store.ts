...
import { SHOPIFY_CART_STORAGE_KEY } from "./cart.constants";

/**
 * Shopify storefront cart.
 *
 * Shopify owns the authoritative cart, so every mutation goes to the
 * Storefront API first and local state only follows on success. The persisted
 * slice is just enough to re-find that remote cart after a reload.
 */
